...
function write(data, waitForANewLine) {
  return new Promise((resolve) => {
    Puck.write(data, resolve, waitForANewLine);
  });
}
...
export function useComms() {
    ...
    write(`\x03\x10reset(${opt == "wipe" ? "1" : ""});\n`).then(
      function rstHandler(result) {
        ...
        if (result == "" && tries-- > 0) {
          ...
          return write(`\x03`).then(rstHandler);
        } else {
  ...
  function uploadApp(app, skipReset) {
    ...
    return AppInfo.getFiles(app, {
      ...
      function doUploadFiles() {
        ...
        if (fileContents.length == 0) {
          return write(`\x10E.showMessage('Hold BTN3\\nto reload')\n`).then(
            (result) => {
              progressBar.hide();
        ...
        function uploadCmd() {
          ...
          // wait for a newline
          return write(`${cmd};Bluetooth.println("OK")\n`, true).then(
            (result) => {
      ...
      // Start the upload
      function doUpload() {
        return write(`\x10E.showMessage('Uploading\\n${app.id}...')\n`).then(
          (result) => {
            if (result === null) {
              progressBar.hide();
  ...
  removeAllApps: () => {
    ...
    return new Promise((resolve, reject) => {
      ...
      function handleResult(result, err) {
        console.log(
          "<COMMS> removeAllApps: received " + JSON.stringify(result)
        );
        if (result == "" && timeout--) {
          console.log(
            "<COMMS> removeAllApps: no result - waiting some more (" +
              timeout +
              ")."
          );
    ...
    return new Promise((resolve, reject) => {
      ...
      connection.cb = function (d) {
        ...
        if (fileSize === undefined) {
          ...
          Progress.show({
            percent: (100 * fileContent.length) / (fileSize || 1000000),
          });
        }
        if (finished) {
          Progress.hide();
      ...
      Bluetooth.print("\xFF");
        ...
        () => {
          Progress.show({
            title: `Reading ${JSON.stringify(filename)}`,
            percent: 0,
          });
